...
class SetMatching(chainer.Chain):
    def __init__(self, n_units, n_encoder_layer=2, n_decoder_layer=2, h=8,
                 n_iterative=2, enc_apply_ln=True, dec_apply_ln=True,
                 dec_component='MAB', loss="k-pair-set"):
...
class SetMatchingCov(SetMatching):
    def __init__(self, n_units, n_encoder_layer=2, n_decoder_layer=2, h=8,
                 n_iterative=2, enc_apply_ln=True, dec_apply_ln=True,
                 dec_component='MAB', loss="k-pair-set", weight='mean',
                 logits=True):

        super(SetMatchingCov, self).__init__(n_units, n_encoder_layer, \
                 n_decoder_layer, h, n_iterative, enc_apply_ln, \
                 dec_apply_ln, dec_component, loss)
        with self.init_scope():
    ...
    def forward_kpair(self, x, x_seq, y, y_seq):
        ...
        # apply weights
        wx = self.estimate_weight(x) # estimate whether x belongs to training data or not
        wy = self.estimate_weight(y) # estimate whether y belongs to training data or not
