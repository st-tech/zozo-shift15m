...
class ConvolutionSentence(L.Convolution2D):
    ...
    def __init__(self, in_channels, out_channels, nobias):
        super(ConvolutionSentence, self).__init__(
            in_channels, out_channels, ksize=1, stride=1, pad=0,
            nobias=nobias, initialW=linear_init, initial_bias=None)
...
class MultiHeadAttention(chainer.Chain):
    ...
    def _relu_activation(self, _batch_A, _mask, h, batch, n_queries, n_keys, xp):
        ...
        batch_A /= n_elements
        batch_A = F.where(
            xp.isinf(batch_A.data), xp.zeros(batch_A.shape, 'f'), batch_A)
        return batch_A
...
class MultiHeadSimilarity(chainer.Chain):
    ...
    def __call__(self, x, z=None, mask=None):
        ...
        n_elements = xp.where(n_elements==0, xp.ones(n_elements.shape, 'f'), n_elements)
...
class MultiHeadExpectation(chainer.Chain):
    ...
    def __call__(self, x, y, yx_mask):
        ...
        batch_xy *= self.scale_score # if self.act_func is nonlinear, you should consider moving this line to before self.act_func
...
class FeedForwardLayer(chainer.Chain):
    def __init__(self, n_units):
        ...
        with self.init_scope():
            self.w_1 = ConvolutionSentence(None, n_inner_units,
                                           nobias=False)
...
class ISAB(chainer.Chain):
    def __init__(self, n_units, h=8, m=16):
        ...
        with self.init_scope():
            ...
            self.self_attention_1 = MultiHeadAttention(n_units, h,
            ...
            self.self_attention_2 = MultiHeadAttention(n_units, h,
                                                       self_attention=False)
...
class MAB(chainer.Chain):
    ...
    """
    def __init__(self, n_units, h=8, activation_fn='relu',
                 apply_ln=True):
        super(MAB, self).__init__()
        with self.init_scope():
            self.attention = MultiHeadAttention(n_units, h,
                                                self_attention=False,
...
class PMA(chainer.Chain):
    ...
    """
    def __init__(self, n_units, h=8, n_output_instances=None,
...
class SetEncoder(chainer.Chain):
    ...
    def get_attnmap(self, e, mask):
        ...
        for name in self.layer_names:
            attnmaps.append(
                getattr(self, name).get_attnmap(e, mask))
            e = getattr(self, name)(e, mask)
        return attnmaps
...
class SetEncoderISAB(chainer.Chain):
    ...
    def get_attnmap(self, e, xi_mask, ix_mask):
        ...
        for name in self.layer_names:
            attnmaps.append(
                getattr(self, name).get_attnmap(e, xi_mask, ix_mask))
            e = getattr(self, name)(e, xi_mask, ix_mask)
        return attnmaps
...
class SetDecoderNoSAB(chainer.Chain):
    def __init__(self, n_units, h=8):
        ...
        with self.init_scope():
            self.rff_1 = FeedForwardLayer(n_units)
            self.mab = MultiHeadAttention(n_units, h,
                                          self_attention=False)
            self.rff_2 = FeedForwardLayer(n_units)
...
class SimpleSetDecoder(chainer.Chain):
    def __init__(self, n_units, h=8, apply_last_rff=True, n_units_last=None,
                 component='MAB', activation_fn='relu', apply_ln=True):
        super(SimpleSetDecoder, self).__init__()
        with self.init_scope():
            self.rff_1 = FeedForwardLayer(n_units)
            if component == 'MAB':
                self.mab = MAB(n_units, h, activation_fn=activation_fn,
                               apply_ln=apply_ln)
...
class StackedSimpleSetDecoder(chainer.Chain):
    def __init__(self, n_units, n_layers=2, h=8, n_units_last=None,
                 component='MAB', activation_fn='relu',
                 apply_ln=True):
        ...
        for i in range(1, n_layers):
            name = 'dec_{}'.format(i)
            layer = SimpleSetDecoder(n_units, h, apply_last_rff=False,
                                     component=component,
                                     activation_fn=activation_fn,
                                     apply_ln=apply_ln)
            self.add_link(name, layer)
        ...
        layer = SimpleSetDecoder(n_units, h, apply_last_rff=True,
    ...
    def __call__(self, z, s, xy_mask):
        if len(s.shape) == 2:
            ...
            s = F.broadcast_to(F.transpose(s, (1, 0)),
                               (batch, n_units, n_candidates))
        for name in self.layer_names:
