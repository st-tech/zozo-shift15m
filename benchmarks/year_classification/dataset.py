...
def get_train_val_dataset(feature_dir, label_dir, year_pos, year_neg, split):
    ...
    train_pos = json.load(open(os.path.join(label_dir, str(year_pos), split, 'train.json')))
    train_neg = json.load(open(os.path.join(label_dir, str(year_neg), split, 'train.json')))
    train_dataset = [{'item_id': str(i), 'label': np.int32(1)} for i in train_pos] + \
                    [{'item_id': str(i), 'label': np.int32(0)} for i in train_neg]
    random.shuffle(train_dataset)
    print('train.json loaded. pos: ' + str(year_pos) + ' neg: ' + str(year_neg) + ' split: ' + split)

    valid_pos = json.load(open(os.path.join(label_dir, str(year_pos), split, 'valid.json')))
    valid_neg = json.load(open(os.path.join(label_dir, str(year_neg), split, 'valid.json')))
    valid_dataset = [{'item_id': str(i), 'label': np.int32(1)} for i in valid_pos] + \
                    [{'item_id': str(i), 'label': np.int32(0)} for i in valid_neg]
    print('valid.json loaded. pos: ' + str(year_pos) + ' neg: ' + str(year_neg) + ' split: ' + split)
    
    return _get_dataset(train_dataset, feature_dir, is_train=True), \
           _get_dataset(valid_dataset, feature_dir, is_train=False)
  

def _get_dataset(sets, feature_dir, is_train=False):
    return TransformDataset(
            LoadData(sets, feature_dir),
            TransformData(is_train))
