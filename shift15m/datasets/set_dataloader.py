...
class get_dataset(object):
    ...
    def get_example(self, i):
        return np.array([self._load_feature(os.path.join(self.feat_dir, j['item_id'] + '.json.gz')) for j in self.sets[i]['items']])
...
def get_train_val_dataset(feature_dir, label_dir, download=True):
    ...
    return get_dataset(train, feature_dir), \
           get_dataset(valid, feature_dir)
